...
@section('contenido')
<div class="max-w-2xl">
    <form method="POST" action="{{ route('marcas.store') }}" class="space-y-5">
        @csrf

        @foreach ([
            ['nombre', 'Nombre', 'text', true],
            ['tono', 'Tono de comunicación', 'text', false],
        ] as [$campo, $label, $tipo, $requerido])
            <div>
                <label class="block text-sm text-gray-400 mb-1">{{ $label }}{{ $requerido ? ' *' : '' }}</label>
                <input type="{{ $tipo }}" name="{{ $campo }}" value="{{ old($campo) }}"
        ...
        @foreach ([
            ...
            <div>
                ...
                <textarea name="{{ $campo }}" rows="3"
                          class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2 text-sm text-gray-100 focus:outline-none focus:border-violet-500 resize-none">{{ old($campo) }}</textarea>
            </div>
        @endforeach
